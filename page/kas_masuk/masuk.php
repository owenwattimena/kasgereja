...
if (isset($_POST['simpan'])) {
    ...
    $sql = $koneksi->query("insert into kas (no_acc, keterangan, tgl, jumlah, jenis, keluar, kode_kas)values('$kode', '$keterangan', '$tgl', '$jml', 'masuk', 0, '$kode_kas')");
...
kas.kode, kas.no_acc, kas.tgl, kas.keterangan, kas.jumlah, kode_kas.kode_kas FROM kas 
    LEFT JOIN kode_kas on kas.kode_kas = kode_kas.id
    WHERE kas.jenis = 'masuk'
                                    ...
                                    <form role="form" method="POST">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="kode_kas">
                                                ...
                                                while ($data = $sql_kode_kas->fetch_assoc()) {
                                                ?>
                                                    <option value="<?= $data['id'] ?>"><?= $data['kode_kas'] ?></option>
                                                <?php
                                                } ?>
                                            </select>
                                        </div>
                        ...
                        <tbody>
                            ...
                            ?>
                                ...
                                <tr class="odd gradeX">
                                    ...
                                    <td>
                                        <a id="edit_data" data-toggle="modal" data-target="#edit" data-id="<?php echo $data['kode'] ?>" data-no_acc="<?= $data['no_acc'] ?>" data-keterangan="<?php echo $data['keterangan'] ?>" data-tgl="<?php echo $data['tgl'] ?>" data-jml="<?php echo $data['jumlah'] ?>" class="btn btn-info"><i class="fa fa-edit"></i>Edit</a>
